...
# Liner Mixed Effects Model for FADV_6 MVAL
# LME P value calculation
meta$TWIN_Pair<-as.factor(meta$TWIN_Pair)
meta$FADV_6<-meta$FADV_6+abs(min(meta$FADV_6))### make the minimum 0
...
save(Likelihood_Ratio_Test, file="Likelihood_Ratio_Test_lmer_FADV_6_PAWS_Mval_genetic_cluster_and_twin_fixed_allFactors.RData")

Likelihood_Ratio_Test_Mval<-sapply(1:length(Likelihood_Ratio_Test), function(x) Likelihood_Ratio_Test[[x]][["Pr(>Chisq)"]][2])

pvalue_dist_fadv_6<-data.frame(CpG=rownames(PAWS_Mval), Nominal_P=Likelihood_Ratio_Test_Mval)
ggplot(pvalue_dist_fadv_6, aes(Nominal_P))+geom_histogram(fill="grey90", color="black")+theme_bw()+xlab("Nominal P Value")+
  ylim(0,50000)
...
delbeta<-sapply(1:nrow(PAWS_Beta), function(x) {
  z<-lm(unlist(PAWS_Beta[x,]) ~ meta$FADV_6 +meta$Genetic_cluster)
...
save(Multi_test_corr_relaxed, stat_hits, delbeta, bio_hits, file="FADV_6_PAWS_hits_Mval.RData")# thresholded on Mval but data in stahits and biohits if betas for plotting
